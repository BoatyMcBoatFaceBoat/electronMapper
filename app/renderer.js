...
inputButton.addEventListener('click', () => {
  ...
  if (pathName) {
    // let result = ipcRenderer.sendSync('async-file-path', pathName);
    // console.log(result);
    // mapBuilder(result);
    ipcRenderer.on('async-reply', (event, arg) => {
      console.log(`reply from renderer: ${arg}`);
      // buildTreeStructure(arg);
      createMapTest()
        .then(buildTreeStructure(arg))
        .catch(err => console.log(err));
      // await buildTreeStructure(arg);
      // await mapBuilder();
      // mapBuilder(arg);
...
function createMapTest(){
  return new Promise((resolve, reject) => {
    // build map with d3
    console.log('build map function');
    const error = false;
  ...
  function buildTreeStructure(line){
    console.log(buildTreeStructure);
